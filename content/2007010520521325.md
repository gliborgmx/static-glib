...
las direcciones de correo que parecían ser ficticias y daban la puntada
de ser bots.
...
pero bajo un dominio de correo en común, por lo que con un SQLazo
podemos ver la cantidad de cuentas que el GLIB tiene registrado.

```console
$ echo "SELECT email FROM users" | mysql geeklog -u root -p \
...
Fuera de las de HotMail, GMail y Yahoo (¿quién usa Hotmail existiendo
...
my @uids = ();
my $getBadUsers = """
SELECT uid FROM users WHERE email RLIKE
(strokersclub.net|sweetsnet.com|yamy.net|dro4ers.net|1stflirt.org|koziavok.net
|lovesnake.net|4watcher.com|pornoscop.com)
""";
...
foreach my $uid (@uids) {
    ...
    foreach my $table qw(users userinfo) {
        my $query = "DELETE FROM $table WHERE uid=$uid";
