...
ssh-keygen
cp .ssh/id_pub .ssh/authorized_keys
for NODE in `cat /etc/machines`; do
  rsh $NODE
  mkdir .ssh
...
Nos conectamos al nodo, instalamos el paquete NFS,
...
Ajustes de hosts. Editamos /etc/hosts para incluir todos los nodos,
agregamos:
...
añadimos las dependencias, agregamos el usuario *sgeadmin* e instalamos
el cliente:
...
Ahora podemos ejercitar el mismo comando sin problemas, pero es
