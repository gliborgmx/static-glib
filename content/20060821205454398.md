...
El día de hoy, después de darme de alta en Enchilame me puse a buscar la
...
Copia la carpeta imágenes en el directorio principal de tu instalación de
...
1. Puedes incluirlo en la página principal (index.php) o en las páginas
   individuales de cada post (single.php, por lo general), debe ir en el loop
   para que aparezca en el lugar que prefieras dentro del post, Con él sólo
   puedes enviar artículos:

```php
<?php enchilame(); ?>
```

1. Sólo lo puedes incluir en la página individual de cada post (single.php, por
   lo general), debe ir en el loop para que aparezca en el lugar que prefieras
   dentro del post. Puedes enviar artículos y votar:

```php
<?php enchila(); ?>
```

Para descargar el plugin lo puedes hacer desde estas URL:

<http://kbyte.us/dwnlds/code/enchilame.zip>

<http://kbyte.us/?page_id=65> para mas códigos.
